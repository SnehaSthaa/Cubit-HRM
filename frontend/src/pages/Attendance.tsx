...
export default function Attendance() {
  ...
  const handleSync = () => {
    ...
    setTimeout(() => {
      ...
      toast({
        title: "Sync complete",
        description:
          "Pulled latest punches from ZKTeco K40 devices via ZKBioAccess.",
      });
    }, 1500);
  };
  ...
  const handleAddDevice = () => {
    if (!newDevice.name || !newDevice.ip) {
      toast({
        ...
        description: "Device name and IP are required.",
        variant: "destructive",
      });
      return;
    }
  ...
  const saveEdit = () => {
    ...
    if (!editDraft.reason.trim()) {
      toast({
        ...
        description: "Please enter a reason for the manual edit.",
        variant: "destructive",
      });
      return;
    }
    const newHours =
      editDraft.checkIn && editDraft.checkOut
        ...
        : editDraft.status === "Absent" || editDraft.status === "On Leave"
          ? "—"
          : editRow.hours;
    ...
    if (editDraft.checkIn !== editRow.checkIn) {
      entries.push({
        ...
        date: "Today",
        field: "Check-in",
        oldValue: editRow.checkIn,
        newValue: editDraft.checkIn || "—",
        editor,
    ...
    if (editDraft.checkOut !== editRow.checkOut) {
      entries.push({
        ...
        date: "Today",
        field: "Check-out",
        oldValue: editRow.checkOut,
        newValue: editDraft.checkOut || "—",
        editor,
        reason: editDraft.reason,
        at: now,
      });
    ...
    if (editDraft.status !== editRow.status) {
      entries.push({
        ...
        date: "Today",
        field: "Status",
        oldValue: editRow.status,
        newValue: editDraft.status,
        editor,
        reason: editDraft.reason,
        at: now,
      });
    ...
    if (entries.length)
      setAuditLog((prev) => [...entries, ...prev].slice(0, 200));
    ...
    toast({
      ...
      description: `${editRow.name}'s record was edited successfully.`,
    });
    setEditRow(null);
  };
  ...
  const handleExport = () => {
    ...
    toast({
      ...
      description: `Downloaded CSV for ${months[Number(selectedMonth)]} ${selectedYear}.`,
    });
  };
  ...
  const filteredMonthly = useMemo(() => {
    return selectedEmployee === "all"
      ? monthlyData
      : monthlyData.filter((r) => r.id === selectedEmployee);
      ...
      <motion.div variants={item} className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Daily logs · monthly reports · ZKTeco K40 biometric integration via
            ZKBioAccess
          </p>
        </div>
            ...
            <Dialog open={configDialog} onOpenChange={setConfigDialog}>
              ...
              <DialogContent className="max-w-2xl">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Connect ZKTeco K40 biometric devices via ZKBioAccess Web
                    API.
                  </DialogDescription>
                </DialogHeader>
                  ...
                  <div className="bg-muted/30 border border-border rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <Input
                          defaultValue="https://zkbio.company.com/api"
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Dialog
                        ...
                        onOpenChange={setAddDeviceDialog}
                      >
                        <DialogTrigger asChild>
                          <Button
                            variant="outline"
                          ...
                          <div className="space-y-3 pt-2">
                            ...
                            <div>
                              ...
                              <Select
                                ...
                                onValueChange={(v) =>
                                  setNewDevice({ ...newDevice, model: v })
                                }
                              >
                                <SelectTrigger className="h-8 text-xs">
                                  <SelectValue />
                            ...
                            <div className="flex justify-end gap-2 pt-1">
                              <Button
                                variant="outline"
                                size="sm"
                                onClick={() => setAddDeviceDialog(false)}
                              >
                                Cancel
                              </Button>
                    ...
                    <div className="space-y-2">
                      {devices.map((device) => (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            <span
                              className={`status-pill ${device.status === "online" ? "status-active" : "status-resigned"}`}
                            >
                              {device.status}
                            </span>
                            <Button
        ...
        >
          <TabsList>
            <TabsTrigger value="daily" className="gap-1.5">
              <CalendarIcon className="w-3.5 h-3.5" /> Daily Log
            </TabsTrigger>
            <TabsTrigger value="monthly" className="gap-1.5">
              <TrendingUp className="w-3.5 h-3.5" /> Monthly Report
            </TabsTrigger>
            <TabsTrigger value="devices" className="gap-1.5">
              <Wifi className="w-3.5 h-3.5" /> Devices
            </TabsTrigger>
          </TabsList>
            ...
            <div className="grid grid-cols-5 gap-4">
              {[
                ...
                <div key={s.label} className="glass-card p-4">
                  ...
                  <p className={`text-2xl font-bold font-mono-data ${s.color}`}>
                    {s.value}
                  </p>
                </div>
              ))}
              ...
              <div className="px-5 py-4 border-b border-border flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[11px] text-muted-foreground mt-0.5">
                    Check-in / Check-out & total time present in office. Click{" "}
                    <Pencil className="w-3 h-3 inline -mt-0.5" /> to edit.
                  </p>
                </div>
                    ...
                    <tr key={row.id}>
                      ...
                      <td className="text-sm font-medium">
                        ...
                        {row.edited && (
                          <span
                            title={row.editNote}
                            className="ml-2 text-[10px] px-1.5 py-0.5 rounded bg-warning/10 text-warning font-medium"
                          >
                            edited
                          </span>
                        )}
                      ...
                      <td className="font-mono-data text-xs font-semibold">
                        {row.hours}
                      </td>
                      <td className="text-xs text-muted-foreground">
                        {row.source}
                      ...
                      <td>
                        <span
                          className={`status-pill ${statusColors[row.status] ?? ""}`}
                        >
                          {row.status}
                        </span>
                      </td>
          ...
          <TabsContent value="monthly" className="space-y-5">
              ...
              {[
                {
                  ...
                  value: monthlyTotals.totalPresent,
                  icon: CheckCircle2,
                  color: "text-success",
                },
                {
                  label: "Late Arrivals",
                  value: monthlyTotals.totalLate,
                  icon: AlertCircle,
                  color: "text-warning",
                },
                {
                  label: "Absent Days",
                  value: monthlyTotals.totalAbsent,
                  icon: AlertCircle,
                  color: "text-destructive",
                },
                {
                  label: "Leave Days",
                  value: monthlyTotals.totalLeave,
                  icon: Clock,
                  color: "text-primary",
                },
              ].map((s) => (
            ...
            <div className="glass-card overflow-hidden">
              <div className="px-5 py-4 border-b border-border">
                ...
                <p className="text-[11px] text-muted-foreground mt-0.5">
                  Total time present, leaves, late arrivals, and absences per
                  employee. Push to Payroll to feed salary calculation.
                </p>
              </div>
              <table className="nexus-table">
                <thead>
                  ...
                  {filteredMonthly.map((row) => {
                    ...
                    return (
                      <tr key={row.id}>
                        ...
                        <td className="text-center font-mono-data text-xs">
                          {row.workingDays}
                        </td>
                        <td className="text-center font-mono-data text-xs font-semibold text-success">
                          {row.present}
                        </td>
                        <td className="text-center font-mono-data text-xs font-semibold text-warning">
                          {row.late}
                        </td>
                        <td className="text-center font-mono-data text-xs font-semibold text-destructive">
                          {row.absent}
                        </td>
                        <td className="text-center font-mono-data text-xs font-semibold text-primary">
                          {row.leave}
                        </td>
                        <td className="font-mono-data text-xs font-semibold">
                          {row.totalHours}
                        ...
                        <td className="font-mono-data text-xs text-accent">
                          {row.overtime}
                        </td>
                      </tr>
                    );
            ...
            <div className="flex items-center justify-between">
              ...
              {isHR && (
                <Button
                  variant="outline"
                  size="sm"
                  className="gap-1 h-7 text-xs press-effect"
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {editRow && (
                <span>
                  {editRow.name} ·{" "}
                  <span className="font-mono-data">{editRow.id}</span>
                </span>
              )}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 pt-1">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEditDraft((d) => ({ ...d, checkIn: e.target.value }))
                  }
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEditDraft((d) => ({ ...d, checkOut: e.target.value }))
                  }
                  className="h-9 font-mono-data"
                />
              </div>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v: DailyRow["status"]) =>
      ...
      <Dialog open={auditDialog} onOpenChange={setAuditDialog}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            ...
            <DialogDescription>
              All manual changes to attendance records are tracked here.
            </DialogDescription>
          </DialogHeader>
                ...
                <tbody>
                  {auditLog.map((a) => (
                    <tr key={a.id}>
                      ...
                      <td className="text-xs font-mono-data text-destructive">
                        {a.oldValue}
                      </td>
                      <td className="text-xs font-mono-data text-success">
                        {a.newValue}
                      </td>
                      <td className="text-xs text-muted-foreground">
                        {a.reason}
