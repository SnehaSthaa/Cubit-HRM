...
// ─── Correction requests are stored in localStorage (shared key) ───
...
export default function Attendance() {
  ...
  const [correctionDialog,       setCorrectionDialog]       = useState(false);   // employee submit dialog
  const [hrRequestsDialog,       setHrRequestsDialog]       = useState(false);   // HR review dialog
  ...
  const fetchDailyLog = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/attendance?date=${selectedDate}&limit=100`,
        { headers: authHeaders() }
      );
      if (!res.ok) { const msg = await extractError(res); throw new Error(msg); }
      const json = await res.json();
  ...
  // ────────────────────────────────────────────────────────────
  // ── Correction Request: Submit (Employee) ──
  // ────────────────────────────────────────────────────────────
  ...
  // ────────────────────────────────────────────────────────────
  // ── Correction Request: Approve / Reject (HR) ──
  // ────────────────────────────────────────────────────────────
  const handleCorrectionAction = (id: string, action: "approved" | "rejected") => {
    const editor = getCurrentUser()?.name ?? "HR";
    const updated = correctionRequests.map((r) =>
      r.id === id ? { ...r, status: action as CorrectionRequestStatus, actionBy: editor } : r
    );
    saveCorrectionRequests(updated);
    setCorrectionRequests(updated);
    toast({
      title: action === "approved" ? "Request approved" : "Request rejected",
      description: action === "approved" ? "Attendance will be updated accordingly." : "Employee will be notified.",
    });
  };
  ...
  // ── Pending correction requests count (for HR badge) ──
  const pendingCorrectionCount = useMemo(
    () => correctionRequests.filter((r) => r.status === "pending").length,
    [correctionRequests]
  );

  // ── My correction requests (for employee: only their own) ──
  ...
  // ── Correction request type label helpers ──
              ...
              <div className="space-y-3 pt-2">
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(v) => setNewDevice({ ...newDevice, model: v })}
                  >
    ...
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
      ...
      <motion.div variants={item} className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {!isHR && (
            <Button
              variant="outline" size="sm" className="gap-1.5 press-effect"
          ...
          {/* ── HR: Requests button with pending badge ── */}
          {isHR && (
            <Button
              variant="outline" size="sm" className="gap-1.5 press-effect"
              onClick={() => setHrRequestsDialog(true)}
            >
              <Send className="w-3.5 h-3.5" /> Requests
            ...
            <Dialog open={configDialog} onOpenChange={setConfigDialog}>
              ...
              <DialogContent className="max-w-2xl">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Manage ZKTeco K40 biometric devices. The device auto-registers on first punch
                    using the serial number you configure on the physical unit.
                  </DialogDescription>
                </DialogHeader>
                {deviceConfigContent}
              </DialogContent>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-5">
          <TabsList>
            ...
            {!isHR && (
              <TabsTrigger value="my_requests" className="gap-1.5">
                <Send className="w-3.5 h-3.5" /> My Requests
              ...
              <div className="flex items-center justify-between gap-6 flex-wrap">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground font-mono-data">
                      {nepalLiveTime.toLocaleTimeString("en-US", {
                        hour: "2-digit", minute: "2-digit", second: "2-digit", timeZone: "UTC",
                      })}
                      {" · "}
                      {nepalLiveTime.toLocaleDateString("en-US", {
                ...
                <div className="flex items-center gap-2">
                  {!myAttendance.checkedIn ? (
                    <Button
                      size="sm" className="gap-2 bg-success hover:bg-success/90 text-white"
                    ...
                    <Button
                      ...
                      onClick={handleCheckOut} disabled={checkInLoading}
                    >
            ...
            <div className="glass-card p-4 flex items-center justify-between gap-3 flex-wrap">
              ...
              {isHR && (
                <p className="text-xs text-muted-foreground">
                  HR/Admin view · all employees · click{" "}
                  <Pencil className="w-3 h-3 inline -mt-0.5 mx-0.5" />
                  to edit
                </p>
              )}
            </div>
          ...
          <TabsContent value="devices" className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              {isHR && (
                <Button
                  variant="outline" size="sm" className="gap-1 h-7 text-xs press-effect"
                    ...
                    <div className="space-y-3">
                      ...
                      <div>
                        ...
                        <Select
                          ...
                          onValueChange={(v) => setAddForm((f) => ({ ...f, device_id: v }))}
                        >
                      ...
                      <Button
                        ...
                        onClick={handleAddMapping} disabled={addMappingLoading}
                      >
                ...
                <div className="flex-1 glass-card overflow-hidden">
                  <div className="px-5 py-4 border-b border-border flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-[11px] text-muted-foreground mt-0.5">
                        {apiMappings.length} mappings across{" "}
                        {devices.filter((d) => d.serial_number !== "MANUAL").length} devices
                      </p>
                    </div>
            ...
            <TabsContent value="my_requests" className="space-y-4">
              ...
              {myCorrectionRequests.length === 0 ? (
                <div className="glass-card p-12 text-center">
                  <Send className="w-8 h-8 text-muted-foreground mx-auto mb-3 opacity-50" />
      ...
      <Dialog open={correctionDialog} onOpenChange={setCorrectionDialog}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Submit a request to HR/Admin for a missed check-in or check-out.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 pt-1">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v: CorrectionRequestType) =>
                    setCorrectionForm((f) => ({ ...f, type: v, checkIn: "", checkOut: "" }))
                  }
                >
            ...
            {/* Check-in time — shown for check_in or both */}
            ...
            {/* Check-out time — shown for check_out or both */}
      ...
      {/* ══ HR: Attendance Correction Requests Dialog ══ */}
      <Dialog open={hrRequestsDialog} onOpenChange={setHrRequestsDialog}>
        <DialogContent className="max-w-4xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Approve or reject employee-submitted check-in / check-out corrections.
            </DialogDescription>
          </DialogHeader>
                ...
                <tbody>
                  {correctionRequests.map((r) => (
                    <tr key={r.id}>
                      ...
                      <td className="text-xs text-muted-foreground max-w-[160px] truncate" title={r.reason}>
                        {r.reason}
                      </td>
                      <td>
                        <div className="flex flex-col gap-0.5">
                          ...
                          {r.actionBy && (
                            <span className="text-[10px] text-muted-foreground">{r.actionBy}</span>
                          )}
                        </div>
                      </td>
                      <td className="text-right">
          ...
          <div className="space-y-3 pt-1">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v: AttendanceStatus) => setEditDraft((d) => ({ ...d, status: v }))}
              >
                <SelectTrigger className="h-9 text-sm"><SelectValue /></SelectTrigger>
                <SelectContent>
          ...
          <DialogFooter>
            ...
            {auditLog.length > 0 && (
              <Button
                ...
                onClick={() => { setAuditLog([]); toast({ title: "Audit log cleared" }); }}
              >
